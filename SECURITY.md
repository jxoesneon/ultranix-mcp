...
| forks | | The project does not maintain backport branches. Security fixes are
...
| Fix - Low | Next regular release | ### Disclosure process
...
| **HTTP, dev mode**| Auth disabled iff `ULTRANIX_MCP_DISABLE_AUTH=true` | Explicit opt-in escape hatch for local development. Logs a loud startup warning and emits an audit event. Never acceptable outside loopback development. | ### Request pipeline (every tool call)
...
| ONNX models | `~/.ultranix-mcp/models/` | SHA-256 checksum verified at download | ### Supply chain
