...
| stdio transport | spawn binary, JSON-RPC over pipes | identical `tools/list` to HTTP golden | Golden regeneration is explicit (`cargo test -- --ignored regenerate_goldens` or
...
| Audit | every call emits exactly one JSONL line with tool, `key_id`, `args_hash` (never raw args), outcome, duration; `prev_hash` chain verification detects a broken or reordered link | ### 1.4 Fuzzing - schema parsing
...
| Degradation drill | restart without `HYPRLAND_INSTANCE_SIGNATURE` / AT-SPI2 bus stopped -> `/readyz` reports `None`; tools return capability-unavailable | ### 2.2 Nested compositor (disposable sessions)
...
| integration - X11 | `Xvfb` job | release-blocking once scheduled (X11 rungs shipped at v1.1.0) | **Miri consideration:**`cargo miri test` runs on the pure-logic subset
