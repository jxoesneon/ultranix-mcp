# ultranix-mcp — Enterprise & Governance Plan (Spec)
...
**Spec phase — describes the approved design and the planned policy roadmap;
not a shipped-features guarantee.**
...
Design rules: **typed text and screenshots are never written to the audit
...
`GET /metrics` on `127.0.0.1:3010` (HTTP mode) exposes the canonical eight
metrics. The table below **mirrors `docs/ARCHITECTURE.md` §7 verbatim** —
that document owns the metric names, types, and labels; this copy exists for
reader convenience and must not diverge:

| Metric | Type | Labels | Description |
| --- | --- | --- | --- |
| `ultranix_mcp_tool_calls_total` | Counter | `tool`, `category`, `result` | Tool call count by outcome |
| `ultranix_mcp_tool_duration_seconds` | Histogram | `tool`, `category` | Per-tool execution latency |
| `ultranix_mcp_auth_failures_total` | Counter | `transport` | Rejected `uxcp_*` key attempts |
...
| `ultranix_mcp_backend_active` | Gauge | `backend` | Which backend won negotiation, per provider |
| `ultranix_mcp_action_history_size` | Gauge | — | Entries in `history.json` |
| `ultranix_mcp_ocr_cache_entries` | Gauge | — | Live OCR cache entries |
...
degradation (e.g. `CaptureProvider` falling from `WlrScreencopy` to portal
or `None`).

Candidate Phase-4+ additions under consideration (not yet canonical):
...
Alert candidates: growth in `ultranix_mcp_auth_failures_total`,
`ultranix_mcp_rate_limit_rejections_total` growth, `result="error"` ratio on
`ultranix_mcp_tool_calls_total` > 5%, a `ultranix_mcp_backend_active`
transition to a weaker backend, and `/readyz` reporting a degraded provider
set versus the wlroots baseline.
...
| --- | --- |
| **CC6.1 — Logical access** | `uxcp_*` bearer auth on HTTP; stdio is spawn-scoped to the owning user; `ULTRANIX_MCP_DISABLE_AUTH` flagged in logs and metrics. |
...
| **CC7.2 — Monitoring/detection** | append-only `audit.jsonl`; denied-call auditing; Prometheus metrics incl. auth-failure and rejection counters. |
...
phone-home, no analytics, Sentry only when `ULTRANIX_MCP_SENTRY_DSN` is
explicitly set. The single built-in network fetch is the **first-call ONNX
...
- ⚠️ Partial: portal fallback works on GNOME/KDE remote sessions but shows
...
| Knob | Status | Notes |
| --- | --- | --- |
| `--category=` tool filtering | **Phase 1** | Startup capability cap; the primary policy lever today. |
| Input sanitization + arg-constrained whitelists | **Phase 1** | Security scaffolding: keysym whitelist, bounds checks, selector-injection rejection, the six-binary `system_command` whitelist with absolute binary pinning (`hyprctl` denies `dispatch exec`/`exec-once`; `xdotool`/`wmctrl` X11-only), and the path whitelist (`$XDG_RUNTIME_DIR`, `/tmp`, `~/.ultranix-mcp/**`). |
| Consent gate | **Phase 1** | Destructive tools (`system_command`, `clear_action_history`, `replay_action`, `window_control` `close`) return `-32015 ConsentRequired` + challenge token; retry with `consent_token`; `--allow-destructive` bypass. |
| Audit skeleton (JSONL append + schema) | **Phase 1** | `audit.jsonl` with `key_id`/`args_hash`/`prev_hash` chaining exists from Phase 1 — security scaffolding, not a Phase-4 add-on. |
| API-key auth + disable flag | **Phase 4** | The HTTP-transport auth surface (`uxcp_*` key check, `X-API-Key`/`Bearer` headers, key files, rotation overlap, auth audit events) lands with the Phase-4 enterprise milestone. Note: fail-closed bind semantics are intrinsic — whenever HTTP is enabled in earlier phases, the server still refuses to bind `:3010` without a key. |
| Rate limiting (10 req/s token bucket per client identity) | **Phase 4** | HTTP-transport feature; `/metrics` and health endpoints exempt; env-tunable budget. |
| Encrypted history + Prometheus + Sentry | **Phase 4** | AES-256-GCM `history.json`, the canonical 8 metrics, `/health`+`/readyz`, optional `ULTRANIX_MCP_SENTRY_DSN` — the enterprise hardening milestone. |
| **Tool whitelists** (`--allow-tools=`, deny-by-default profiles) | Phase 4 | Finer than category: e.g. serve `screenshot` but not `type_text`. |
| **Read-only mode** (`--readonly`) | Phase 4 | Serves only `vision` + non-mutating `admin` tools; every input tool is denied (`outcome=denied`, `denial_reason=readonly_mode`). The "observe but don't touch" deployment for incident-analysis agents. |
| **Approval gates (beyond the Phase-1 consent gate)** | Phase 4–5 | Out-of-band confirmation for sensitive calls — e.g. text typed into password-focused fields — pluggable: local libnotify/dunst prompt on the desktop, or a webhook to an approver service. Builds on, does not replace, the `-32015 ConsentRequired` challenge. |
| **Per-key scoping** | Phase 5 | `uxcp_*` keys carry a category/whitelist claim, so one daemon serves a read-only analyst key and a full-access automation key simultaneously. |
| **Config file policy** (`~/.config/ultranix-mcp/policy.toml`) | Phase 5 | Declarative: whitelists, rate budgets, approval-gate rules, audit redaction level. Fleet-managed via Ansible/Nix. |
...
- [ ] Approval-gate UX: local desktop prompt vs webhook — design during
      Phase 4.
...
*Spec · companion to `docs/PACKAGING.md` and `docs/MARKET_ANALYSIS.md`.*
