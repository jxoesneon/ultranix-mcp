...
**Version**: 0.1.0 — **Status**: Specification phase
...
- Startup probe: if uinput is the selected backend, emit an explicit
  `backend.uinput.active` audit event + stderr warning and set the
  `ultranix_mcp_backend_active{backend="uinput"}` gauge.
...
| **R-7** | uinput mode enables physical-equivalent input to privileged prompts (sudo/polkit) | Medium–High when active | Required fallback where virtual-input protocols unavailable | Prefer Wayland virtual input; dedicated udev group; alert on `ultranix_mcp_backend_active{backend="uinput"}` |
