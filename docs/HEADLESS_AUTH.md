...
| A *different* local UID | Cannot open your stdio pipes - DAC blocks it | **The boundary this does NOT create:**an authenticated-but-malicious *agent*
...
| LAN-exposed `0.0.0.0:3010` | - | **Unsupported.**No TLS + bearer keys + full desktop control = do not. | A Unix-socket listener removes the entire "who else can reach the port" class
...
| Action history, audit, metrics | internal | | | **Bottom line for true headless hosts:**with no `WAYLAND_DISPLAY`/`DISPLAY`
