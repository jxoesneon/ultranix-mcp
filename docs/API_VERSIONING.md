...
| **MCP protocol version**| The wire protocol negotiated with the client | only when the MCP spec revs | `initialize` -> `protocolVersion` (negotiated) | Server version and tool-surface version move together (the tool surface is
...
| `1.x` | Active | full guarantees above; security + critical fixes ship as patch releases | ## Version Metadata
