...
| **Metrics**| Tool-call counts, latencies, error rates | In-memory, exposed at `/metrics` | None (counters only, no content) | Process lifetime | ### Notes on the sensitive items
...
| Everything else | - | **None**| - | There is no update checker, no license phone-home, no feature-flag service.
